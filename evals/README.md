...
- Gateway running on `:8108` with keys in repo-root `.env`:
...
- Run sync **from this directory** (`evals/`), not the repo root:
  ```bash
  cd evals
  uv sync
  ```
- KB present at `agent/sandbox/kb/` (chunked at startup)
...
Upload a file like `agent/state/chats/<id>/conversation.json`:
...
per-turn scores + averages, and saves `evals/results/eval-<timestamp>.json`.
