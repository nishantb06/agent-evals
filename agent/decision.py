...
def _format_hits(hits: list[MemoryItem]) -> str:
    ...
    # For classifier facts (`value.raw`) and indexed-chunk facts
    # (`value.chunk`), the body IS the answer material — stripping it
    # leaves Decision unable to synthesise. We render a short chunk
    # preview here so Decision can answer directly from the memory-hit
    # list when search_knowledge has already populated it.
    if not hits:
        return "  (none)"
    out = []
    for h in hits[:10]:
...
def _format_history(history: list[dict]) -> str:
    ...
    for h in history[-6:]:
        ...
        if kind == "answer":
            ...
            tool = h.get("tool")
            # Clip result_descriptor at 300 chars so the model sees
            # multi-entry tool outputs (e.g. list_dir) without hiding
            # the tail that a tighter clip would cut off.
