...
USER_QUERY. Upstream results appear under INPUTS. When CHAT HISTORY
is present, use it only to resolve references and keep continuity —
the answer must still address the current USER_QUERY. When PERSONA
is present, match its tone without inventing facts.
...
Rules:
  ...
  - The answer must be answerable from INPUTS alone (plus CHAT HISTORY
    for pronoun/continuity resolution). If an upstream
