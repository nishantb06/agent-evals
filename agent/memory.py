...
keyword overlap. Writes embed the descriptor at insert time for items of
kind `fact`, `preference`, and `tool_outcome`. Scratchpad items skip
embedding.
...
Design choices:
  1. Vector retrieval only for now. Hybrid retrieval with RRF is a
     future enhancement.
  2. Sliding-window chunking inside `index_document` is heuristic;
     semantic chunking is a future enhancement.
...
def _index() -> VectorIndex:
    ...
    Re-reading the index file is cheap at typical scale and keeps the agent
...
def remember(
    ...
    *,
    ...
    # The classifier at temp=1.0 sometimes returns an
    # empty `value` dict, discarding the only
