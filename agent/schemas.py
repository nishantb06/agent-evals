"""Typed contracts every layer in the agent talks in.
...
`MemoryItem` optionally carries an `embedding`. Items of kind `fact`,
`preference`, and `tool_outcome` carry a vector embedding written by Memory
at insert time. The embedding underlies FAISS vector search. Items of kind
`scratchpad` are run-scoped and skip embedding.
"""
...
class NodeState(BaseModel):
    ...
    for replay — it shows the exact bytes that hit the gateway, not a
    reconstruction."""
