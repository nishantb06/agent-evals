...
# Skills that receive bounded multi-turn dialogue. Workers stay scoped to
# their Planner-assigned inputs so fan-out isolation is preserved.
_CHAT_CONTEXT_SKILLS = frozenset({"planner", "formatter"})
...
def render_prompt(skill: Skill, query: str, resolved: list[dict],
    ...
    # Persona + chat history only on Planner/Formatter — see plan.
    if skill.name in _CHAT_CONTEXT_SKILLS:
        if isinstance(persona, str) and persona.strip():
            parts += ["", f"PERSONA:\n{persona.strip()}"]
        if isinstance(chat_context, str) and chat_context.strip():
            parts += ["", "CHAT HISTORY (previous turns):", chat_context.strip()]
    ...
    # Same hits flow into every skill's prompt this run so every cognitive
    # role can see what the agent already knows.
...
async def run_skill(skill: Skill, node_id: str, graph_nodes,
                    ...
                    *, memory_hits: list | None = None,
                    chat_context: str | None = None,
                    persona: str | None = None) -> tuple[AgentResult, str]:
    ...
    skill's prompt can see the same hits.

    `chat_context` / `persona` are multi-turn chat wrappers; only
    Planner and Formatter receive them in the rendered prompt.
    ...
    rendered = render_prompt(
        skill, query, resolved, failure_report,
    ...
    # Malformed successors used to be silently dropped, which left
    # "missing node" bugs hard to diagnose. Now: log the offending JSON
    # + the validation error, then fail the node so the failure path
    # (and replay) surfaces it.
