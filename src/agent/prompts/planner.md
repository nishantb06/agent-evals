...
Available skills:
  ...
  (browser           reserved for browser automation)
...
If CHAT HISTORY appears in the prompt, it lists previous completed
turns of this conversation. USER_QUERY is the NEW message that must
be answered now — resolve pronouns and references ("that", "it",
"the previous answer") against CHAT HISTORY before planning. Prefer
a `formatter` (optionally with MEMORY HITS) when history already
contains the material needed; do not redo prior research/work unless
the user asks for an update or expansion.

If PERSONA appears in the prompt, honour its tone and role when
choosing what the formatter should produce, without inventing new
skills.
