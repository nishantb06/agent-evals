...
We open one MCP stdio session per skill invocation (the spawn cost is
~100ms and the session lives only for the lifetime of one node —
keeping it short means no shared mutable state between skills).
