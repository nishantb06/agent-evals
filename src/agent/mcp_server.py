"""
MCP server for the Ollive agent.
...
def list_dir(path: str = ".") -> dict:
    ...
    # A list[dict] return was being rendered as one MCP TextContent per
    # entry. After clipping and downstream slicing, only the first 2-3
    # file dicts survived into the Decision prompt, and Decision then
    # declared the directory complete at whatever it could see. Returning
    # a single dict with `count` and a flat `names` list keeps the
    # cardinality visible even under truncation.
    p = _safe(path)
    entries = []
    names: list[str] = []
...
def _chunk_text(text: str, size: int = 400, overlap: int = 80) -> list[str]:
    """Sliding-window chunking by word count. Semantic chunking is a
    future enhancement."""
    words = text.split()
    if not words:
        return []
