...
```bash
curl -s http://localhost:8108/v1/embedders | python3 -m json.tool
```

That endpoint lists the configured embedders and the fixed dim. If it returns 404 or refuses connection, the gateway isn't up yet:

```bash
cd gateway
./run.sh                 # creates .venv on first run, then starts on port 8108
...
The agent architecture has four cognitive layers — **Perception → Memory → Decision → Action**. The first three all need an LLM to do their work, and which LLM is right for the job depends on what kind of work it is. A simple "what's the capital of France?" perception step doesn't need Gemini 2.5; a 5,000-token memory digest does. Without routing, you either over-pay on small queries or under-perform on big ones.
...
4. **You can read the code and verify the wall holds.** `_classify_tier()` physically constructs the bounded envelope; the router providers never receive anything else. The principle is enforced in 30 lines.
...
Edit `../.env` (or `.env` beside the gateway dir):
...
No other changes needed — the dashboard, `/v1/routers`, and call logging all pick it up automatically.
...
- **No new tools / no agent loop.** The gateway is the substrate; the agent loop that uses `auto_route` lives in `../agent/`.
...
1. **The router can be turned off.** Skip `auto_route` and the gateway behaves like a plain provider pool. The router is opt-in, not load-bearing.
2. **The wall is enforced in code, not by convention.** `_classify_tier()` constructs the bounded envelope; the router providers never receive anything else.
3. **Free-tier reality forces design choices.** Provider rate limits and deprecations are constraints to design around — the four-router pool is the response.
4. **Token thresholds are honest, not aspirational.** The 1K elbow is set where free-tier small models actually start failing; the 8K ceiling is set where free-tier Gemini quality actually starts dropping. Test these elbows with your own keys.
5. **`fallback_used` is the honest measure of routing health.** A high `fallback_used` count means the router pool is overloaded; the gateway didn't lie about its decisions, it just couldn't make them with a real LLM.
