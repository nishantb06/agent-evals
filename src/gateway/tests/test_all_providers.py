#!/usr/bin/env python3
"""Per-provider matrix test for the LLM gateway.

Tests A (basic), B (tools), C (structured), D (caching), E (reasoning)
against each of the 7 providers. Prints a matrix at the end.

Assumes the gateway is running at http://localhost:8108
(env LLM_GATEWAY_URL to override).
"""
from __future__ import annotations
import os, sys, json, time, httpx

URL = os.getenv("LLM_GATEWAY_URL") or os.getenv("LLM_GATEWAY_V2_URL", "http://localhost:8108")
